  ...
  IF NOT EXISTS (
    ...
    WHERE version = '007_payload_storage_identity'
      ...
      (version = '008_namespace_binding_timeline_identity' AND checksum IS DISTINCT FROM
        '32ddcf4da2aefe3e417c11080b8e7dc62d6515bd7d0e42b005caac965261e7bd')) THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM reference_schema_migrations WHERE version = '008_namespace_binding_timeline_identity') AND (EXISTS (SELECT 1 FROM reference_metadata_timeline) OR EXISTS (SELECT 1 FROM reference_metadata_observations)) THEN
    RAISE EXCEPTION 'migration 008 requires the TypeScript reference migrator';
  END IF;
END
$$;

INSERT INTO reference_schema_migrations(version, checksum) VALUES ('008_namespace_binding_timeline_identity', '32ddcf4da2aefe3e417c11080b8e7dc62d6515bd7d0e42b005caac965261e7bd')
