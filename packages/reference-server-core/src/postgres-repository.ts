...
import { PostgresContractReleaseRepository } from "./postgres/postgres-contract-release-repository.js";
import { PostgresEndpointRepository } from "./postgres/postgres-endpoint-repository.js";
import { PostgresPayloadCleanupRepository } from "./postgres/postgres-payload-cleanup-repository.js";
import { PostgresPayloadNamespaceRepository } from "./postgres/postgres-payload-namespace-repository.js";
import { PostgresPayloadReferenceRepository } from "./postgres/postgres-payload-reference-repository.js";
import { PostgresPayloadRepository } from "./postgres/postgres-payload-repository.js";
import {
  PostgresRepositoryContext,
  type PostgresReferenceRepositoryOptions,
} from "./postgres/postgres-repository-context.js";
import { PostgresSecretRepository } from "./postgres/postgres-secret-repository.js";
import { PostgresTestCommandRepository } from "./postgres/postgres-test-command-repository.js";
import { PostgresTimelineAuditRepository } from "./postgres/postgres-timeline-audit-repository.js";
import type {
  ReferenceRepository,
  AuditRecord,
...
export type { PostgresReferenceRepositoryOptions } from "./postgres/postgres-repository-context.js";

export class PostgresReferenceRepository implements ReferenceRepository {
  readonly #ctx: PostgresRepositoryContext;
  readonly #contractRelease: PostgresContractReleaseRepository;
  readonly #endpoint: PostgresEndpointRepository;
  readonly #secret: PostgresSecretRepository;
  readonly #testCommand: PostgresTestCommandRepository;
  readonly #timelineAudit: PostgresTimelineAuditRepository;
  readonly #payload: PostgresPayloadRepository;

  constructor(options: PostgresReferenceRepositoryOptions) {
    this.#ctx = new PostgresRepositoryContext(options);
    this.#ctx.attach(this);
    this.#contractRelease = new PostgresContractReleaseRepository(this.#ctx);
    this.#endpoint = new PostgresEndpointRepository(this.#ctx);
    this.#secret = new PostgresSecretRepository(this.#ctx);
    this.#testCommand = new PostgresTestCommandRepository(this.#ctx);
    this.#timelineAudit = new PostgresTimelineAuditRepository(this.#ctx);
    this.#payload = new PostgresPayloadRepository(
      new PostgresPayloadReferenceRepository(this.#ctx),
      new PostgresPayloadCleanupRepository(this.#ctx),
      new PostgresPayloadNamespaceRepository(this.#ctx),
    );
  }

  get pool(): Pool {
    return this.#ctx.pool;
  }
